...
interface SidebarMenuActionProps extends React.ComponentPropsWithoutRef<typeof Button> {
  asChild?: boolean;
  showOnHover?: boolean;
}
...
const SidebarMenuBadge: React.FC<SidebarMenuBadgeProps> = ({
  ...
  return (
    <div
      ...
      className={cn(
        "text-sidebar-foreground pointer-events-none absolute right-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-primary p-1 text-xs font-medium text-primary-foreground opacity-0 transition-all group-data-[collapsible=icon]:opacity-100 group-data-[collapsible=icon]:group-hover/menu-item:opacity-0 group-data-[collapsible=icon]:group-focus-within/menu-item:opacity-0",
        className
      )}
      {...props} />
  );
...
interface SidebarMenuLinkProps extends React.ComponentPropsWithoutRef<typeof Slot> {
  asChild?: boolean;
}
...
const SidebarMenuLink: React.FC<SidebarMenuLinkProps> = ({
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute inset-0 outline-hidden focus-visible:ring-2 focus-visible:ring-sidebar-ring",
        "group-data-[collapsible=icon]:hidden"
      )}
      {...props} />
  );
...
const SidebarOverlay: React.FC<SidebarOverlayProps> = ({
  ...
  return (
    <div
      ...
      className={cn(
        "bg-background/80 fixed inset-0 z-40 hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 md:hidden",
        className
      )}
      {...props} />
  );
